    ...
    <div class="container-fluid">
      <div class="row pt-3">
        <div class="col-md-12">
          ...
          <div class="row indiv-Arrangement-wrap">
            <div
            ...
            >
              <figure
                class="effect-sadie"
                @click="setSelectedArrangement(index)"
              >
                <img
                ...
                >
                  <md-field>
                    ...
                    <md-input
                      ...
                      type="number"
                      class="qty-input"
                      @change="setSelectedArrangement(selectedLevel)"
                    />
                  </md-field>
                </div>
                <div
            ...
            </div> <!-- end col-md-4 v-for -->
          </div> <!-- end row indiv-Arrangement -->
        </div> <!-- end col-md-12 -->
      </div> <!-- end row pt-3 -->
    </div> <!-- end container-fluid -->
  </div> <!-- end outer template div FINAL -->
</template>
...
export default {
  ...
  data() {
    return {
      the_prod_id: this.info[
        this.$store.state.selectedConfig.initialCollection.collection
      ]['prod_id'],
      selectedLevel: "",
      qty: 0
    }
  ...
  computed: {
    ...
    selected_level(){
      return this.$store.state.activeSelections[this.arrangementName][0].level
    },
    selectedArrangement: {
      get() {
        return this.$store.state.activeSelections[this.arrangementName][0].level_index
      }
    },
    totalCost(){
      let total = this.$store.state.activeSelections[this.arrangementName][0].price * this.qty
      return total
    },
    ...
    setSelectedArrangement(newVal) {
      ...
      let obj = {
        ...
        this.$store.dispatch('activeSelections/setActiveCenterpieceSelection', obj, { root: true }).then(()=>{
          this.profileUpdateMixin('multi', this.arrangementName)
        })
    },

    initializeQty(){
      // console.log(parseInt(this.$store.state.activeSelections[this.arrangementName][0].qty) )
      if (this.$store.state.activeSelections[this.arrangementName][0].qty === "" ){
          this.qty = 0
          // console.log("NOTHING FOR QTY")
        } else {
          this.qty = this.$store.state.activeSelections[this.arrangementName][0].qty
        }
    },

    initializeIndex(){
      // console.log(parseInt(this.$store.state.activeSelections[this.arrangementName][0].level_index) + " " + this.arrangementName)
      if (this.$store.state.activeSelections[this.arrangementName][0].level_index === "" ){
          this.selectedLevel = 1
          // console.log("NOTHING FOR INDEX OF " + this.arrangementName)
        } else {
          this.selectedLevel = this.$store.state.activeSelections[this.arrangementName][0].level_index
        }
    },
  }
}
